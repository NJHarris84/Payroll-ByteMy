  ...
  try {
    ...
    switch (operation) {
      case 'get_all_clients':
        const clientsResult = await adminClient.query({ query: GET_ALL_CLIENTS });
        return NextResponse.json({ success: true, clients: clientsResult.data.clients });
      
      case 'get_all_payrolls':
        const payrollsResult = await adminClient.query({ query: GET_ALL_PAYROLLS });
        return NextResponse.json({ success: true, payrolls: payrollsResult.data.payrolls });
      
      case 'get_all_staff':
        const staffResult = await adminClient.query({ query: GET_ALL_STAFF });
        return NextResponse.json({ success: true, staff: staffResult.data.staff });
      
      case 'get_all_users':
        const usersResult = await adminClient.query({ query: GET_ALL_USERS });
        return NextResponse.json({ success: true, users: usersResult.data.users });
      
      case 'update':
        if (!table || !id || !data) {
          return NextResponse.json({ error: "Missing required parameters" }, { status: 400 });
        }
        const updateResult = await adminClient.mutate({
      ...
      case 'delete':
        ...
        const deleteResult = await adminClient.mutate({
      ...
      case 'insert':
        ...
        const insertResult = await adminClient.mutate({
