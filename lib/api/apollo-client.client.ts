...
import { tokenManagerClient } from "@/lib/auth/token-manager.client"; // Use client-specific token manager
...
export function getClientApolloClient(): ApolloClient<NormalizedCacheObject> {
  ...
  const authLink = setContext(async (_, { headers }) => {
    try {
      ...
      const token = await tokenManagerClient.getToken(); // Use client-specific token manager
...
export const apolloClient = getClientApolloClient();
